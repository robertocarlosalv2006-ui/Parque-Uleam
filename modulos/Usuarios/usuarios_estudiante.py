...
def menu_registro_buscar_estudiante(ventana):
    ...
    # Botón Volver optimizado sin usar imports complejos en línea
    ...
    tk.Button(
        ...
        bg="snow",
        fg="black",
        command=lambda: Registrar_Estudiante(entry_nombre, entry_cedula),
        cursor="hand2"
    ).pack(pady=5)
    ...
    tk.Button(
        ...
        bg="snow",
        fg="black",
        command=lambda: buscar_Estudiante(entry_buscar, resultado_text),
        cursor="hand2"
    ).pack(pady=3)
...
def Registrar_Estudiante(entry_nombre, entry_cedula):
    ...
    # CORREGIDO: Inserción directa en la base de datos real usando tu función de database.py
    id_generado = insertar_usuario(cedula, nombre)
    
    if id_generado is not None:
        messagebox.showinfo("Éxito", "Estudiante registrado correctamente en la Base de Datos")
        entry_nombre.delete(0, tk.END)
        entry_cedula.delete(0, tk.END)
    else:
        messagebox.showerror("Error", "La cédula ya se encuentra registrada en el sistema")

def buscar_Estudiante(entry_buscar, resultado_text):
    ...
    # CORREGIDO: Consulta real a SQLite sobre la tabla única de usuarios
    try:
        conexion = sqlite3.connect(DB_NAME)
        cursor = conexion.cursor()
        
        # Busca si coincide exactamente con la cédula o si el nombre contiene el texto ingresado
...
def regresar_a_usuarios(ventana):
    # Desacoplamiento limpio para evitar importaciones circulares complejas en los lambdas
